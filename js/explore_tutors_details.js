  $(function() {
    ...
    $('.time-option, .day-option').on('click', function(e) {
      ...
      // Teacher hover
    $('#teachers_listing .teacherSection').hover(
        function () {
          const $schedule = $('#schedule-panel');
          const $listing = $('#teachers_listing');
          const $section = $(this);

          const sectionTop = $section.offset().top;
          const listingTop = $listing.offset().top;

          const translateY = sectionTop - listingTop;
...
// video play
    ...
    function render(page) {
      ...
      // refer to
       $('.teacherSection').on('click', function (e) {
      // Prevent redirect when clicking interactive elements
      if (
        $(e.target).closest('a, button, input, textarea, select, .see-more').length
        ) {
          return;
        }

        window.location.href = '../tutorprofile/my_lessons_tutor_profile.php';
      });
      // refer to

      // filter sticky
      $('#page').scroll(function() {
        var currentScrollTop = $(this).scrollTop(); // Get current scroll position
            // Scrolling up
            if (currentScrollTop > 230){
                $(".filters_box").addClass("fiter_shadow")
              }
              else {
              $(".filters_box").removeClass("fiter_shadow")
